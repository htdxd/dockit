...
from skill_toolbox.contracts.resume import (
    ResumeContentV2, ResumeEditV2, ResumeGenerateV2Request, ResumeHeaderV2, ResumeRepairV2Request,
    ResumePersonalField,
)
...
def test_t001_profile_and_named_body_ids():
    profile = get_profile("t001")
    assert profile.header_slot_order == ("role", "org", "date")
    assert profile.header_labels["手机"] == "phone"
    assert profile.header_labels["年龄"] == "age"
    root = emit.load_document_xml(TEMPLATE)
    assert t001.anchor_by_id(root, 8) is not t001.anchor_by_id(root, 20)
    assert t001.source_key({"id": "projects"}) == "work"


def test_explicit_heading_without_date_uses_heading_style():
    root = emit.load_document_xml(TEMPLATE)
    body = t001.body_for(root, {"id": "projects"})
...
@pytest.mark.parametrize("unfit", [False, True])
def test_header_fit_uses_available_gap_or_explicitly_fails(tmp_path, monkeypatch, unfit):
    closed = []

    class Shape:
        def __init__(self, left_column):
            self.Left = 57.6 if left_column else 255.85
            self.Width = 176.75 if left_column else 179.4
            self.Top, self.Height = 139.75, 99.45
            self.left_column = left_column

        @property
        def TextFrame(self):
            return SimpleNamespace(
                Overflowing=self.left_column and (unfit or self.Width < 190),
                TextRange=SimpleNamespace(Paragraphs=SimpleNamespace(Count=0)))

    shapes = {"ResumeHeader22": Shape(True), "ResumeHeader62": Shape(False)}
    ...
    monkeypatch.setitem(sys.modules, "win32com",
                        SimpleNamespace(client=SimpleNamespace(DispatchEx=lambda _: word)))
    if unfit:
        with pytest.raises(ValueError, match="HEADER_OVERFLOW"):
            t001.fit_header(TEMPLATE, {"target_role": "无法放下的长内容"}, tmp_path)
    else:
        fitted = t001.fit_header(TEMPLATE, {"target_role": "AI Agent 应用 / CV 算法"}, tmp_path)
        assert fitted["22"]["width_pt"] == pytest.approx(191.05)
        assert fitted["22"]["height_pt"] == 99.45
        assert fitted["22"]["overflow"] is False
    assert closed == ["doc", "word"]
...
def test_only_page_decorations_may_repeat_for_multiple_pages():
    banner = "求职简历 PERSONAL RESUME 我一直在努力！"
    entries = [{"entry_id": "a", "text": "正文条目甲"}, {"entry_id": "b", "text": "正文条目乙"}]
    pages = {1: banner + "姓名：自定义姓名" + "正文条目甲", 2: banner + "正文条目乙"}
    allowed = t001.fixed_texts(TEMPLATE, pages=2) + ["自定义姓名"]
    assert not qa.check_no_extra_rendered_content(pages, entries, allowed_extra_needles=allowed)
    pages[2] += "自定义姓名"
    assert qa.check_no_extra_rendered_content(pages, entries, allowed_extra_needles=allowed)
...
def test_formatted_measurement_host_preserves_font_size_and_bullets(tmp_path, monkeypatch):
    seen = []
    ...
    def measure(entries):
        for entry in entries:
            ...
            paragraphs = body.findall(".//" + emit.W + "txbxContent/" + emit.W + "p")
            seen.append([
                (p.find("./" + emit.W + "r/" + emit.W + "rPr/" + emit.W + "sz").get(emit.W + "val"),
                 p.find("./" + emit.W + "pPr/" + emit.W + "numPr") is not None)
                for p in paragraphs
            ])
            assert entry["body_item"] is None
        return []

    monkeypatch.setattr(pipeline.measure, "measure_documents", measure)
    svc = ResumeEditService(tmp_path, TEMPLATES, template_id="t001")
    scenario = svc._scenario_from_content(content().model_dump())
    pipeline.measure_scenario(scenario, tmp_path, template=TEMPLATE, template_id="t001")
    assert seen == [[("21", False), ("20", True)]]
...
def test_insert_section_before_is_explicit_and_missing_target_is_rejected(tmp_path):
    ...
    changed, _ = svc._apply_changes(original, [
...
@pytest.mark.e2e
def test_t001_real_generation(tmp_path):
    svc = ResumeEditService(tmp_path, TEMPLATES, capabilities={"vision": True}, template_id="t001")
    result = svc.generate(ResumeGenerateV2Request(template_id="t001", content=content(),
                                                 request_id="t001-real"))
    record = svc.store.load_revision(result.artifact_id, result.revision)
    assert record["mechanical"]["passed"], record["mechanical"]["errors"]
    assert record["page_count"] == 1
    assert record["content"]["template_id"] == "t001"
