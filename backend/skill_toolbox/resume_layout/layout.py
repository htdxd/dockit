...
def entry_body_height(m: MeasureResult) -> float:
    """条目正文框高度 = 首行偏移 + 文字高 + 底部余量。"""
    return BODY_FIRST_TEXT_OFF_PT + m.text_height_pt + BODY_BOTTOM_PAD_PT


def _entry_footprint(k: int, body_h: float) -> float:
    ...
    """
    return (BODY_ANCHOR_OFF_PT if k == 0 else 0.0) + body_h
...
def plan_layout(
    ...
    *,
    ...
    """
    capacity_h = page_bottom_pt - page_top_pt
    ...
    default_entry_gap = DEFAULT_ENTRY_GAP_PT
    ...
    for si, sec in enumerate(sections):
        ...
        for k, ent in enumerate(entries):
            ...
            footprint = _entry_footprint(k, body_h)
            ...
            if cursor is None:
                ...
                place = y + dy
                if place < page_top_pt - 1e-6:
                    raise LayoutUnsatisfiable(
                        ent["id"],
                        f"上移后位置 {place:.1f}pt 超出内容区上边界 {page_top_pt:.1f}pt",
                    )
        ...
        prev_box_bottom = max(
            (e.anchor_y_pt + e.body_h_pt for e in sec_entries), default=None
        )
        text_bottom = (
...
def section_ink_bottom_by_page(plans: list[SectionPlan]) -> list[dict[int, float]]:
    """每栏每页的**文字底**（最后一条 ink 底），用于跨栏目碰撞判定。"""
    out: list[dict[int, float]] = []
    for plan in plans:
        per: dict[int, float] = {}
        for entry in plan.entries:
            bottom = entry.anchor_y_pt + BODY_FIRST_TEXT_OFF_PT + entry.text_height_pt
...
def check_section_flow(
    ...
    """
    ink = section_ink_bottom_by_page(plans)
    for i in range(1, len(plans)):
        nxt = plans[i]
        prev_ink = ink[i - 1].get(nxt.page_index)
        if prev_ink is None or prev_ink == float("-inf"):
            continue
        title_top = nxt.anchor_y_pt + TITLE_TEXT_TOP_OFF_PT
