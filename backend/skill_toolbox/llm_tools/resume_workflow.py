...
_DESCRIPTIONS = {
    ...
    "resume_generate": "将准备好的个人信息与栏目条目生成简历候选并返回渲染图。"
        "项目首行依次为名称 organization、紧凑源码链接、Stars/Forks、性质 role；不显示项目日期，tech_stack 单独在下一行，text 是后续职责和成果。教育/工作日期照常显示。"
        "details 是用户提供的项目指标或链接；每项含 label/value、可选 link 和 emphasis(normal/bold/accent/bold_accent)。"
        "personal_fields 同样支持 link 和 emphasis，适合突出 GitHub/作品集，不用替换现有标签冒充新字段。"
        "highlights 强调字段或正文片段：field=organization/role/date/tech_stack/text，正文另填从0开始的 paragraph 和精确 text；emphasis 选 bold/accent/bold_accent，background=true 加浅底纹。"
        "其他 details 的 layout=inline 可紧凑同行；GitHub 源码默认简写 owner/repo 放在项目名称后，保留可点击完整地址。"
    ...
    "resume_edit": "基于最新 candidate_id 批量编辑内容并返回新候选及渲染图。"
        ...
        "update_entry.entry.highlights 可指定字段或正文原文片段强调，省略保留，[] 撤销全部；改字导致原片段不再唯一匹配时，同时更新 highlights。"
