...
describe("controller event ownership", () => {
  ...
  it("updates capability badges from the probe and keeps manual toggles available", async () => {
    ...
    const provider = settings._defaultProvider({ api_key: "test-key", model: "unknown", reasoning_level: "balanced", vision_override: false, tool_calling_override: true });
    ...
    const send = vi.fn(async (_message: Record<string, unknown>) => {});
    const controller = createProviderController(send);
    ...
    field("btn-probe").addEventListener.mock.calls.find(([event]: string[]) => event === "click")[1]();
    const probe = send.mock.calls.map(([message]) => message).find(message => message.type === "probe_capabilities")!;
    controller.handleEvent({ id: String(probe.id), event: { type: "capabilities_probed", report } });
  ...
  it("edits the model directly and opens its list only with the arrow", () => {
    ...
    field("#btn-fetch-models").handlers.click();
    controller.handleEvent({ id: String(send.mock.calls.at(-1)![0].id), event: { type: "models_fetched", models: ["listed-model"] } });
  ...
  it("leaves unrelated model and task events for the task controller", () => {
    ...
    expect(providers.handleEvent({ id: "models-fetch-1", event: { type: "models_fetched", error: "offline" } })).toBe(true);
